...
export const subscribeToCustomerProfile = (
  ...
  error => {
    onError(error instanceof Error ? error : new Error('Unable to load customer profile.'));
  },
);
...
export const subscribeToStaffProfile = (
  ...
  error => {
    onError(error instanceof Error ? error : new Error('Unable to load staff profile.'));
  },
);
