...
export const DeliveryNavbar = <T extends string,>({
  ...
  <nav className="app-bottom-nav-shell fixed bottom-0 left-0 right-0 z-[80] px-4 pt-2.5 sm:px-6">
    <div className={`app-bottom-nav-card mx-auto grid w-full max-w-[320px] gap-2 rounded-[28px] p-2 shadow-[0_18px_40px_rgba(0,0,0,0.18)] backdrop-blur-xl ${items.length === 1 ? 'grid-cols-1' : items.length > 3 ? 'grid-cols-4' : 'grid-cols-2'}`}>
      {items.map(item => {
