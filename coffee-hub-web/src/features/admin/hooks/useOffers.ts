/**
 * Canonical location for useOffers.
 * Moved here from src/hooks/useOffers.ts to follow feature-based architecture.
 */
...
export const useOffers = ({ includeInactive = false }: UseOffersOptions = {}): UseOffersResult => {
  ...
  const assertCouponCodeUnique = useCallback(async (couponCode: string, offerIdToIgnore = '') => {
    ...
    const hasDuplicate = duplicateSnapshot.docs.some(d => d.id !== offerIdToIgnore);
    if (hasDuplicate) throw new Error('Coupon code already exists.');
  }, []);
  ...
  const findActiveOfferByCode = useCallback(async (couponCode: string) => {
    ...
    try {
      ...
      const shouldFallback = err instanceof FirebaseError && err.code === 'failed-precondition';
    ...
    if (matchingOfferSnapshot.empty) return null;
