...
const sendError = (response: VercelResponse, error: unknown) => {
  ...
  console.error('Unhandled user sync error', error);
  response.status(500).json({ error: 'Unable to sync your profile right now.' });
};
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    const requestUser = await verifyRequestUser(request);
