...
import type { AdminMenuFormState } from '../types';
import { INITIAL_MENU_FORM_STATE } from '../types';

export const useAdminMenuManager = () => {
  const [menuItems, setMenuItems] = useState<MenuItem[]>([]);
  const [menuForm, setMenuForm] = useState<AdminMenuFormState>(INITIAL_MENU_FORM_STATE);
  ...
  const resetForm = () => {
    setMenuForm(INITIAL_MENU_FORM_STATE);
    setEditingId('');
  };
